...
public:
    ...
    {
        dsu ds(n);
        int cnt=0;//It represent number of connections that is not required so we can remove to it.
        for(auto x:connections)
        {
            if(ds.find_parent(x[0])==ds.find_parent(x[1]))
               cnt++;
            else
